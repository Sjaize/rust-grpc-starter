...
use prg_01_hello_gRPC::hello_grpc_pb2::MyNumber; // message class
use prg_01_hello_gRPC::hello_grpc_pb2_grpc::my_service_server::{MyService, MyServiceServer}; // client & server class

// (3) 원격 호출될 함수들을 import 함
use prg_01_hello_gRPC::my_func; // original remotely called functions

// (4) protoc가 생성한 Servicer 클래스를 base class로 해서 원격 호출될 함수들을 멤버로 갖는 서버 클래스를 생성함
struct MyServiceServicer;

#[tonic::async_trait]
impl MyService for MyServiceServicer {
    // (5) 서버 클래스에 원격 호출될 함수에 대한 rpc 함수를 작성함

    // (5.1) proto 화일내 정의한 rpc 함수 이름에 대응하는 멤버 함수를 작성함
    async fn my_function(
        &self,
        request: tonic::Request<MyNumber>,
    ) -> Result<tonic::Response<MyNumber>, tonic::Status> {
        // (5.2) proto 화일내 message 이름과 동일한 message class를 생성하여 응답 전달 용도로 사용함
        let mut response = MyNumber { value: 0 };

        // (5.3) proto 화일내 message 이름과 동일한 message class의 변수에 원격 함수의 수행 결과를 저장함
        // 앞서 (3)의 원격 호출할 함수에게 client로 부터 받은 입력 파라메타를 전달하고 결과를 가져옴
        response.value = my_func(request.into_inner().value);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = server.add_service(MyServiceServer::new(MyServiceServicer));
